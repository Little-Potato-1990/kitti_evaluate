...
bool eval(string gt_dir, string result_dir, Mail *mail) {
  ...
  for (int32_t i = 11; i < 22; i++) {
    ...
    // plot status
    mail->msg("Processing: %s, poses: %d/%d", file_name, poses_result.size(),
              poses_gt.size());
...
int32_t main(int32_t argc, char *argv[]) {
  ...
  string result_dir = "";
  if (argc == 1) {
    ground_truth_dir = "../../ground_truth_poses";
    result_dir = "../../results";
  } else if (argc == 3 || argc == 5) {
    ground_truth_dir = argv[1];
    result_dir = argv[2];
  } else {
    cout << "input param is not correct";
    return 1;
  }
  ...
  // run evaluation
  bool success = eval(ground_truth_dir, result_dir, mail);
  if (argc == 4)
    mail->finalize(success, "odometry", result_dir, argv[2]);
  else
    mail->finalize(success, "odometry", result_dir);
